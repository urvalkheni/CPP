#include <iostream>
#include <string>
#include <vector>
#include <cstring>
#include <iomanip>
#include <sstream>
using namespace std;

// SHA-256 Hash Algorithm - Cryptographic hash function
class SHA256 {
private:
    static const uint32_t K[64];
    
    uint32_t rightRotate(uint32_t n, uint32_t d) {
        return (n >> d) | (n << (32 - d));
    }
    
    uint32_t rightShift(uint32_t n, uint32_t d) {
        return n >> d;
    }
    
public:
    string hash(const string& input) {
        // Initialize hash values (first 32 bits of fractional parts of square roots)
        uint32_t h0 = 0x6a09e667;
        uint32_t h1 = 0xbb67ae85;
        uint32_t h2 = 0x3c6ef372;
        uint32_t h3 = 0xa54ff53a;
        uint32_t h4 = 0x510e527f;
        uint32_t h5 = 0x9b05688c;
        uint32_t h6 = 0x1f83d9ab;
        uint32_t h7 = 0x5be0cd19;
        
        // Pre-processing
        vector<uint8_t> message(input.begin(), input.end());
        uint64_t originalLength = message.size() * 8;
        
        message.push_back(0x80);
        while ((message.size() % 64) != 56) {
            message.push_back(0x00);
        }
        
        for (int i = 7; i >= 0; i--) {
            message.push_back((uint8_t)((originalLength >> (i * 8)) & 0xff));
        }
        
        // Process message in 512-bit chunks
        for (size_t offset = 0; offset < message.size(); offset += 64) {
            uint32_t w[64];
            
            for (int i = 0; i < 16; i++) {
                w[i] = ((uint32_t)message[offset + i * 4] << 24) |
                       ((uint32_t)message[offset + i * 4 + 1] << 16) |
                       ((uint32_t)message[offset + i * 4 + 2] << 8) |
                       ((uint32_t)message[offset + i * 4 + 3]);
            }
            
            for (int i = 16; i < 64; i++) {
                uint32_t s0 = rightRotate(w[i - 15], 7) ^ rightRotate(w[i - 15], 18) ^ rightShift(w[i - 15], 3);
                uint32_t s1 = rightRotate(w[i - 2], 17) ^ rightRotate(w[i - 2], 19) ^ rightShift(w[i - 2], 10);
                w[i] = w[i - 16] + s0 + w[i - 7] + s1;
            }
            
            uint32_t a = h0, b = h1, c = h2, d = h3;
            uint32_t e = h4, f = h5, g = h6, h = h7;
            
            for (int i = 0; i < 64; i++) {
                uint32_t S1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
                uint32_t ch = (e & f) ^ ((~e) & g);
                uint32_t temp1 = h + S1 + ch + K[i] + w[i];
                uint32_t S0 = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22);
                uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
                uint32_t temp2 = S0 + maj;
                
                h = g;
                g = f;
                f = e;
                e = d + temp1;
                d = c;
                c = b;
                b = a;
                a = temp1 + temp2;
            }
            
            h0 += a;
            h1 += b;
            h2 += c;
            h3 += d;
            h4 += e;
            h5 += f;
            h6 += g;
            h7 += h;
        }
        
        stringstream ss;
        ss << hex << setfill('0');
        ss << setw(8) << h0 << setw(8) << h1 << setw(8) << h2 << setw(8) << h3;
        ss << setw(8) << h4 << setw(8) << h5 << setw(8) << h6 << setw(8) << h7;
        
        return ss.str();
    }
};

// Constants (first 32 bits of fractional parts of cube roots of first 64 primes)
const uint32_t SHA256::K[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

int main() {
    cout << "=== SHA-256 Hash Function ===" << endl << endl;
    
    SHA256 sha256;
    
    vector<string> testStrings = {"hello", "world", "SHA256", "cpp", "hash"};
    
    cout << "SHA-256 Hashes:" << endl;
    cout << left << setw(20) << "Input" << "Hash" << endl;
    cout << string(80, '-') << endl;
    
    for (const auto& str : testStrings) {
        cout << setw(20) << str << sha256.hash(str) << endl;
    }
    
    cout << "\nNote: Even small changes in input produce completely different hashes (avalanche effect)" << endl;
    
    return 0;
}
